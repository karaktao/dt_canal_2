...
import { listVNDSlock } from "@/api/infrastructure/VNDSlock";
...
async function attachMapEvents(map) {
  const features = [];
const pageSize = 200;
  const firstPage = await listVNDSlock({ pageNum: 1, pageSize });
  ...
  for (let i = 2; i <= totalPages; i++) {
    pagePromises.push(listVNDSlock({ pageNum: i, pageSize }));
  }
  ...
  // console.log("✅ 全部水闸数据获取完成，共：", allRows.length);
  ...
  lockSource.clear();
  lockSource.addFeatures(features);

  // …保持原有的鼠标悬浮/点击事件绑定…
  emit("map-layer-ready", lockLayer);
}
