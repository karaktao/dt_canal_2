...
service.interceptors.request.use(config => {
  ...
  if (!isRepeatSubmit && (config.method === 'post' || config.method === 'put')) {
    ...
    if (requestSize >= limitSize) {
      console.warn(`[${config.url}]: ` + 'The request data size exceeds the allowed 5MB limit, preventing duplicate submission verification.')
      return config
    }
...
service.interceptors.response.use(res => {
    ...
    if (code === 401) {
      if (!isRelogin.show) {
        isRelogin.show = true
        ElMessageBox.confirm('Login session has expired. You can stay on this page or log in again.', 'System Notic', { confirmButtonText: 'Log in again', cancelButtonText: 'Cancel', type: 'warning' }).then(() => {
